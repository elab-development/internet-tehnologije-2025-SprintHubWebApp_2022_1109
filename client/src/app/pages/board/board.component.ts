...
import { Ticket, Stage, CreateTicketDto, Priority, TicketType, Tag, SprintStatus } from '../../core/models';
      ...
      @if (sprintsService.loading()) {
        ...
        <div class="bg-white border-b border-gray-200 px-4 py-3 flex items-center gap-4">
          ...
          @if (sprint.status === 'PLANNING') {
            <button (click)="changeSprintStatus('ACTIVE')"
                    [disabled]="statusChanging()"
                    class="px-3 py-1.5 text-sm font-medium rounded-lg bg-green-600 text-white
                           hover:bg-green-700 disabled:opacity-50 transition-colors">
              {{ statusChanging() ? 'Starting...' : 'Start Sprint' }}
            </button>
          } @else if (sprint.status === 'ACTIVE') {
            <button (click)="changeSprintStatus('COMPLETED')"
                    [disabled]="statusChanging()"
                    class="px-3 py-1.5 text-sm font-medium rounded-lg bg-blue-600 text-white
                           hover:bg-blue-700 disabled:opacity-50 transition-colors">
              {{ statusChanging() ? 'Completing...' : 'Complete Sprint' }}
            </button>
          } @else if (sprint.status === 'COMPLETED') {
            <button (click)="changeSprintStatus('PLANNING')"
                    [disabled]="statusChanging()"
...
export class BoardComponent implements OnInit, OnDestroy {
  ...
  changeSprintStatus(newStatus: SprintStatus) {
    const sprint = this.sprintsService.currentSprint();
    if (!sprint) return;
    this.statusChanging.set(true);
    this.sprintsService.updateStatus(sprint.id, newStatus).subscribe({
      next: () => {
      ...
      error: (err) => {
        this.statusChanging.set(false);
        this.errorMessage.set(err.error?.message || 'Error changing sprint status');
      },
    });
  }
