...
- **Dashboard** - Pregled radnih prostora, aktivnih sprintova, mojih tiketa i prekoračenih rokova
...
- Panel sa detaljima tiketa za izabrani dan (kod, naslov, prioritet, rok, assignee, stage)
...
- Tipovi notifikacija:
  ...
  - `DEADLINE_WARNING` - Kada se rok tiketa bliži ili je istekao
...
│   ├── src/
│   │   ├── config/             # Environment, database, email, swagger
...
Notification
├── id, userId → User
├── type (TICKET_ASSIGNED | SPRINT_STARTED | DEADLINE_WARNING)
...
| Method | Endpoint | Opis |
|--------|----------|------|
| GET | `/` | Lista tiketa (filteri: q, priority, type, stageId, sprintId, workspaceId, dueDate, assignedToMe, hasDueDate) |
...
| Method | Endpoint | Opis |
|--------|----------|------|
| GET | `/list` | Lista korisnika za dropdown (assignee) |
| GET | `/` | Lista svih korisnika — paginirano (ADMIN) |
| GET | `/:id` | Detalji korisnika (ADMIN) |
| PATCH | `/:id/role` | Promena uloge korisnika (ADMIN) |
| DELETE | `/:id` | Obriši korisnika (ADMIN) |
...
| DELETE | `/cleanup/old` | Brisanje pročitanih starijih od 90 dana (ADMIN) |
...
# Database (SQL Server)
DATABASE_URL="sqlserver://localhost:1433;database=sprinthub;user=sa;password=YourStrong!Password123;encrypt=true;trustServerCertificate=true"
...
SMTP_FROM=SprintHub <noreply@sprinthub.com>
```
...
| Jelena Todorovic | `jelena@sprinthub.test` | `Moderator1` | MODERATOR |
| Marko Petrovic | `marko@sprinthub.test` | `Password1` | USER |
| Ana Jovanovic | `ana@sprinthub.test` | `Password1` | USER |
| Stefan Nikolic | `stefan@sprinthub.test` | `Password1` | USER |

**Preporučeni test scenario:**
1. Prijavi se kao `marko@sprinthub.test` / `Password1`
...
6. Prijavi se kao `ana@sprinthub.test` / `Password1` i proveri notifikacije (bell icon)
7. Otvori vremensku liniju (`/timeline`) i pregledaj tikete po datumima
8. Otvori dashboard (`/dashboard`) za pregled statistike
9. Prijavi se kao ADMIN (`djordjevicdimitrije147@gmail.com` / `Profesija1`) i otvori Admin panel

---

## Docker Deployment

Projekat uključuje `docker-compose.yml` sa 3 servisa:

| Servis | Image / Build | Port |
|--------|---------------|------|
| **db** | `mcr.microsoft.com/mssql/server:2019-latest` | 1433 |
| **backend** | Build: `./server` (node:20-alpine) | 8081 |
| **frontend** | Build: `./client` (multi-stage: node:20-alpine → nginx:alpine) | 80 |

```bash
# Kreiraj .env sa DB_PASSWORD i JWT_SECRET
echo "DB_PASSWORD=YourStrong!Password123" > .env
echo "JWT_SECRET=your_super_secret_jwt_key_minimum_32_characters" >> .env

# Pokreni sve servise
docker compose up -d --build
```

Aplikacija je dostupna na `http://localhost`. Nginx servira Angular build i proksira `/api` i `/api-docs` ka backendu.
...
│   ├── database.js                # Prisma klijent instanca
...
│   ├── user.routes.js             # Lista korisnika + admin CRUD
│   └── notification.routes.js     # CRUD notifikacija + mark read + cleanup
